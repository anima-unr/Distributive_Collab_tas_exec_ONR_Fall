...
public:
	...
	//item_ = it_.subscribe("chatter", 1, &skinDetector::imageCallback2, this);
		...
		image_sub_ = it_.subscribe("/camera/rgb/image_color", 1, &skinDetector::imageCallback, this);
...
printf("centersss : %d %d %d %d\n",center_of_rect.x,center_of_rect.y,img_out.rows,img_out.cols);
ros::NodeHandle n;
		...
		srv.request.x = center_of_rect.x;
		srv.request.y = center_of_rect.y;
...
if(client.call(srv)){
		...
		printf("center : %f %f %f\n",srv.response.newX,srv.response.newY,srv.response.newZ);

			msg1.x=srv.response.newX;
		msg1.y=srv.response.newY;
		msg1.z = srv.response.newZ;
		msg1.x2d = center_of_rect.x;
		msg1.y2d = center_of_rect.y;
